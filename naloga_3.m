A = [1.2094    0.4998    0.9372    1.3284    0.7304    0.4561    1.2465    1.4024    1.1405    0.7302; 
0.4998    1.5025    1.0958    0.8576    0.7749    1.0531    1.4098    0.6539    0.5700    1.1523; 
0.9372    1.0958    0.5086    1.6451    0.2975    1.7235    0.4338    1.1585    1.1617    1.2381; 
1.3284    0.8576    1.6451    1.1705    1.0805    1.2284    0.5148    0.7618    0.9356    0.7314; 
0.7304    0.7749    0.2975    1.0805    1.5583    1.4625    1.0432    1.3437    0.7332    0.5252; 
0.4561    1.0531    1.7235    1.2284    1.4625    0.3313    0.7544    1.0803    0.7996    1.0910; 
1.2465    1.4098    0.4338    0.5148    1.0432    0.7544    1.9016    1.4070    1.1322    0.1541; 
1.4024    0.6539    1.1585    0.7618    1.3437    1.0803    1.4070    0.1689    1.2691    0.4998; 
1.1405    0.5700    1.1617    0.9356    0.7332    0.7996    1.1322    1.2691    1.1594    0.6732; 
0.7302    1.1523    1.2381    0.7314    0.5252    1.0910    0.1541    0.4998    0.6732    0.3678];

[p, q] = maxindex(A);
[c, s] = jacobi_cs(A, p, q);
disp(c);

n = size(A, 1);
Ap = A;

for i = 1:n
    for j = i+1:n
        if abs(Ap(i, j)) > off(A)/n
            [c, s] = jacobi_cs(Ap, i, j);
            R = [c -s; s c];
            Ap(:,[i j]) = Ap(:,[i j]) * R;
            Ap([i j],:) = R' * Ap([i j],:);
        end
    end
end

disp(norm(A - Ap));